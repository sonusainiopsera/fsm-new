...
/**
 * Notification consumer for {@code APPOINTMENT_CONFIRMED_CHANGED} events (BR-05).
 *
 * <p>Notifies the customer when a confirmed appointment is rescheduled or cancelled.
 * No dependency on dispatch.internal — reads portal users by work order ID.
 */
@Profile("worker")
@Configuration
class AppointmentChangedConsumer {

    private static final Logger log = LoggerFactory.getLogger(AppointmentChangedConsumer.class);

    static final String CONSUMER_NAME = "AppointmentChangedConsumer";
    static final String EVENT_TYPE    = "APPOINTMENT_CONFIRMED_CHANGED";
    static final String TEMPLATE_KEY  = "appointment_changed";
    static final String CATEGORY      = "APPOINTMENT_UPDATE";
    ...
    AppointmentChangedConsumer(NotificationPort notificationPort,
    ...
    void consume(DomainEvent event) throws Exception {
        ...
        try {
            rendered = templateRenderer.render(TEMPLATE_KEY, NotificationChannel.IN_APP, "en",
                    Map.of("workOrderRef", workOrderId, "appointmentDate", appointmentDate));
        } catch (Exception ex) {
        ...
        for (UUID recipientUserId : customers) {
            try {
                ...
                log.debug("appointment_changed_idempotent event_id={} recipient={}",
                        event.eventId(), recipientUserId);
            } catch (Exception ex) {
                log.warn("appointment_changed_send_failed event_id={} recipient={}",
    ...
    @Bean
    EventHandler appointmentChangedNotificationHandler() {
