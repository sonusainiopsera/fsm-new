...
- **Files:** 13 (+2006/-0)
- **Duration:** 540ss
- **Approach:** Implemented the technician copilot PWA surface as a bottom-anchored sheet using an explicit state machine (idle/streaming/complete/refused/degraded/capped/partial). The useCopilotStream hook encapsulates the single-use ticket exchange, EventSource lifecycle, chunk accumulation in a Map keyed by chunkIndex (de-duplicated, sorted for out-of-order safety), rAF-batched state flushes for INP, and all terminal event handlers. CopilotBasisList renders an accessible collapsible disclosure (aria-expanded) listing the asset and contributing prior work orders. CopilotHelpfulnessRating uses a TanStack Query mutation for the rating POST with optimistic update and rollback. CopilotSheet is composed entirely from design-token CSS with a sticky Advisory label, fixed-height skeleton region for CLS prevention, named terminal states, and full ARIA accessibility (dialog role, live region, focus-to-close on mount, Escape dismissal, 44px targets). The entry point is gated behind a server-reported copilotEnabled capability flag via useCopilotCapability. Security: model output is rendered only as text nodes — no dangerouslySetInnerHTML anywhere, stripControlChars strips C0/C1 control characters.
